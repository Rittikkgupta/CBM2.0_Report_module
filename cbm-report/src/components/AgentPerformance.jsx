...
function AgentReportTable({
  ...
  const columns = [
    ...
    { id: "date", label: "Date" },
    { id: "agentId", label: "Agent ID" },
    { id: "extnNum", label: "Extension ID" },
    { id: "skillSetName", label: "Skilset Name" },
    { id: "totalNoOfCallsInitiated", label: " Total Calls " },
    { id: "totalNoOfCallsConnected", label: "Call Answered" },
    { id: "totalNoOfCallsFailed", label: "Call Abandoned" },
    // { id: "maxQueueTime", label: "Max Queue Time" },
    { id: "totalTalkTime", label: "Total Talk Time" },
    { id: "totalHandlingTime", label: "Total Handling Time" },
    { id: "percentCallsAnsweredInServiceLevel", label: "Percent Call Answered" },
    { id: "averageSpeedOfAnswer", label: "Average Speed of Answer" },
    { id: "averageTalkTime", label: "Average Talk Time" },
    { id: "averageAbandonCalls", label: " Abandon Calls" },
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.post(
          "http://192.168.45.59:8080/cbmreports/api/reports/agentReport",
          requestData
        );
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.45.59:8080/cbmreports/api/reports/downloadagentReport",
        requestData,
      ...
      link.href = url;
      link.setAttribute("download", "agent_performance_report.csv"); // Set default file name
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } catch (error) {
      console.error("Error downloading agent_performance_report :", error);
    }
  };
